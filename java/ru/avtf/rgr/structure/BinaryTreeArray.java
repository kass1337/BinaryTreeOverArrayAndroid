...
public class BinaryTreeArray implements Serializable {
    ...
    private void scanList(int current, int level, boolean boolTree, ArrayList<String> str){
        if (current >= size)
            return;
        if (arrayTree.get(current) == null)
            return;

        scanList(2 * current + 1, level + 1, boolTree,str);

        if (boolTree) {
            for (int i = 0; i < level; i++)
                str.add("\t\t\t");
            str.add(arrayTree.get(current).toString() + "\n");
        }
        else
            str.add(arrayTree.get(current).toString()+ " ");

        scanList(2 * current + 2, level + 1, boolTree, str);
    }
    ...
    public boolean getDataAtIndexBool(int searchIndex){
        Object obj;
